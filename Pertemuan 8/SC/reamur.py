...
class Reamur:
    ...
    def get_celsius(self):
        return (5/4 * self.suhu)

    def get_fahrenheit(self):
        return (9/4 * self.suhu) + 32

    def get_kelvin(self):
        return (5/4 * self.suhu) + 273

class TemperatureConverterApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Converter Suhu Reamur")
        self.label_reamur = tk.Label(root, text="Reamur:")
        self.label_reamur.grid(row=0, column=0, padx=10, pady=10, sticky="e")

        self.entry_reamur = tk.Entry(root)
        self.entry_reamur.grid(row=0, column=1, padx=10, pady=10)
    ...
    def convert(self):
        try:
            reamur = float(self.entry_reamur.get())
            R = Reamur(reamur)
